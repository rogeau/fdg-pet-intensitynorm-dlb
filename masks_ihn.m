function masks_ihn(patient_dir, control_dir, threshold, contrast)
    if nargin < 3
        threshold = 0.01;
    ...
    if strcmp(contrast, 't')
        ihn_col = sprintf('ihn_%s_t', threshold_str);
    else
        ihn_col = sprintf('ihn_%s', threshold_str);
    end

    if ~ismember(ihn_col, voxel_table.Properties.VariableNames)
        voxel_table.(ihn_col) = NaN(height(voxel_table),1);
    end    
    ...
    patient_paths = dir(fullfile(patient_dir, '**', 's_hn_wr_petsuv.nii'));
    control_paths = dir(fullfile(control_dir, '**', 's_hn_wr_petsuv.nii'));
    ...
    for d = 1:numel(patient_paths)
        ...
        if strcmp(contrast, 't')
            target_folder = fullfile(parent_folder, sprintf('ihn_mask_analysis%s_t', threshold_str));
            if exist(target_folder,'dir')
                rmdir(target_folder,'s'); % remove existing to avoid overwrite prompt
            end
            mkdir(target_folder);
            ...
            matlabbatch{4}.spm.stats.results.export{1}.binary.basename = 'hypo_mask';
            matlabbatch{4}.spm.stats.results.export{2}.ps = true;
    
            original_mask = fullfile(target_folder, 'mask.nii');
            hypo_hyper_mask = fullfile(target_folder, 'spmT_0001_hypo_mask.nii');
    
            matlabbatch{5}.spm.util.imcalc.input = {original_mask; hypo_hyper_mask};
            matlabbatch{5}.spm.util.imcalc.output = sprintf('ihn_mask%s_t', threshold_str);
            ...
            original_mask = fullfile(target_folder,'mask.nii');
            ihn_mask = fullfile(parent_folder, sprintf('ihn_mask%s_t.nii', threshold_str)); % matches your imcalc output
            Vorig = spm_vol(original_mask);
            Yorig = spm_read_vols(Vorig) > 0;
            Vihn = spm_vol(ihn_mask);
            Yihn = spm_read_vols(Vihn) > 0;
    
            vox_removed = nnz(Yorig) - nnz(Yihn);
        ...
        elseif strcmp(contrast, 'f') 
            target_folder = fullfile(parent_folder, sprintf('ihn_mask_analysis%s', threshold_str));
            if exist(target_folder,'dir')
                rmdir(target_folder,'s'); % remove existing to avoid overwrite prompt
            end
            mkdir(target_folder);
            ...
            matlabbatch{5}.spm.util.imcalc.input = {original_mask; hypo_hyper_mask};
            matlabbatch{5}.spm.util.imcalc.output = sprintf('ihn_mask%s', threshold_str);
            matlabbatch{5}.spm.util.imcalc.outdir = {parent_folder};
            matlabbatch{5}.spm.util.imcalc.expression = 'i1 - i2';
            matlabbatch{5}.spm.util.imcalc.var = struct('name', {}, 'value', {});
            matlabbatch{5}.spm.util.imcalc.options.dmtx = 0;
            ...
            original_mask = fullfile(target_folder,'mask.nii');
            ihn_mask = fullfile(parent_folder, sprintf('ihn_mask%s.nii', threshold_str)); % matches your imcalc output
            Vorig = spm_vol(original_mask);
            Yorig = spm_read_vols(Vorig) > 0;
            Vihn = spm_vol(ihn_mask);
            Yihn = spm_read_vols(Vihn) > 0;
        ...
        if isempty(row_idx)
            ...
            new_row.IPP = IPP;
            new_row.Date = Date;
            new_row.(ihn_col) = vox_removed;
            voxel_table = [voxel_table; new_row];
        else
            voxel_table.(ihn_col)(row_idx) = vox_removed;
        end
    end
