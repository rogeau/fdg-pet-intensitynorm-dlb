function masks_ips(patient_dir, control_dir, threshold, contrast)
    if nargin < 3
        threshold = 0.01;
    ...
    if strcmp(contrast, 't')
        ips_col = sprintf('ips_%s_t', threshold_str);
    else
    ...
    for d = 1:numel(patient_paths)
        ...
        if strcmp(contrast, 't')
            ...
            target_folder = fullfile(parent_folder, sprintf('ips_mask_analysis%s_t', threshold_str));       
            ...
            matlabbatch{5}.spm.util.imcalc.output = sprintf('ips_mask%s_t', threshold_str);
            ...
            original_mask = fullfile(target_folder, 'mask.nii');
            ips_mask = fullfile(parent_folder, sprintf('ips_mask%s_t.nii', threshold_str));
